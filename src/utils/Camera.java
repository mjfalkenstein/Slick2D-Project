...
public class Camera {
	...
	public void translate(GameContainer gc, Graphics g, Entity e) {
		...
		if(e.getCenterX()-gc.getWidth()/2+16 < 0){
			dX = 0;
		}else if(e.getCenterX()-gc.getWidth()/2+16 > levelWidth){
			dX = -levelWidth+gc.getWidth();
		}else{
			dX = (int)-e.getCenterX()+gc.getWidth()/2-16;
		}

		//checking that the player is within bounds of the camera vertically
		if(e.getCenterY()-gc.getHeight()/2+16 < 0){
			dY = 0;
		}else if(e.getCenterY()+gc.getHeight()/2+16 > levelHeight){
			dY = -levelHeight+gc.getHeight();
		}else{
			dY = (int)-e.getCenterY()+gc.getHeight()/2-16;
		}
